...
% Initialize panorama as double
panorama = zeros([pHeight, pWidth, 3], 'double'); % Use 'double' explicitly for compatibility
weights = zeros(pHeight, pWidth, 'double'); % Ensure weights are also double

viewpoint = imref2d([pHeight, pWidth], [floor(min(maxX(:))), ceil(max(maxX(:)))], [floor(min(maxY(:))), ceil(max(maxY(:)))]);

for i = 1:numImages
    % Warping image into panorama coordinates
    warpedImg = imwarp(img{i}, projective2d(HRef{i}'), 'OutputView', viewpoint); 
    warpedImg = im2double(warpedImg);
    % Creating a mask of the warped image
    mask = imwarp(true(size(img{i}, 1), size(img{i}, 2)), projective2d(HRef{i}'), 'OutputView', viewpoint);
    % Computing weight map for current image
    distanceToEdge = bwdist(~mask); % Distance from non-mask pixels
    maxDistance = max(distanceToEdge(:));
    weightMap = distanceToEdge / maxDistance; % Normalize weights
    % Expanding weight map to 3 channels to match image dimensions
    weightMap = repmat(weightMap, [1, 1, size(panorama, 3)]);
    % Updating panorama and weights
    panorama = panorama + warpedImg .* weightMap;
    weights = weights + mask .* weightMap(:, :, 1); % Using only one channel for weight accumulation
end

% Normalize the panorama to account for accumulated weights
panorama = panorama ./ max(weights, 1e-6); % Avoid division by zero
...
figure;
imshow(panorama);
title(['Panorama With Weighted Average Blending' ...
    '']);
