...
for i = 1:numImages
   ...
   img{i} = imread(fullfile('data/raw/CMU2', sprintf('medium0%d.jpg',  i)));
...
for i = 1:numImages
    % Warping image to panorama coordinates
    ...
    distanceTransform = bwdist(~mask(:,:,1));
    weightMap = distanceTransform ./ max(distanceTransform(:));
...
panorama = resultPanorama ./ max(weightSum, 1e-6); % Prevents division by zero


% Display the final panorama
figure;
imshow(panorama);
title('Panorama with Feathering Blending');
